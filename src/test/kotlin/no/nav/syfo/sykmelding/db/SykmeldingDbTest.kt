...
class SykmeldingDbTest : FunSpec({
    ...
    context("Leser sykmeldinger fra DB") {
        ...
        test("henter ikke sykmeldinger for pasienter som ikke er registrert i med behandlingsutfall") {
            testDb.insertSymelding(sykmeldingId, fnr, getSykmelding())
            testDb.insertStatus(sykmeldingId, getStatus(StatusEventDTO.SENDT.name, OffsetDateTime.now()))
            testDb.insertStatus(sykmeldingId, getStatus(StatusEventDTO.APEN.name, OffsetDateTime.now().minusDays(1)))
            testDb.insertSykmeldt(fnr)
        ...
        test("henter ikke sykmeldinger for pasienter som ikke er registrert med sykmeldingstatus") {
            testDb.insertSymelding(sykmeldingId, fnr, getSykmelding())
            testDb.insertSykmeldt(fnr)
            testDb.insertBehandlingsutfall(sykmeldingId, getBehandlingsutfall(RegelStatusDTO.OK))
            val sykmeldinger = sykmeldingDb.getSykmeldinger(fnr)
        ...
        test("henter flere sykmeldinger") {
            (0..10).forEach {
                ...
                testDb.insertStatus(id, getStatus(StatusEventDTO.APEN.name, OffsetDateTime.now().minusDays(1)))
                testDb.insertBehandlingsutfall(id, getBehandlingsutfall(RegelStatusDTO.OK))
            }
            testDb.insertSykmeldt(fnr)
        ...
        test("hente nyeste status for sykmelding") {
            ...
            testDb.insertSymelding(sykmeldingId, fnr, getSykmelding())
            testDb.insertStatus(sykmeldingId, getStatus(StatusEventDTO.APEN.name, timestamp))
            testDb.insertBehandlingsutfall(sykmeldingId, getBehandlingsutfall(RegelStatusDTO.OK))
            testDb.insertSykmeldt(fnr)
            ...
            val sporsmalOgSvar = listOf(SporsmalDTO(tekst = "tekst", shortName = ShortNameDTO.ARBEIDSSITUASJON, svar = SvarDTO(SvartypeDTO.ARBEIDSSITUASJON, "")))

            testDb.insertStatus(sykmeldingId, getStatus(StatusEventDTO.SENDT.name, timestamp.plusSeconds(1), arbeidsgiver, sporsmalOgSvar))

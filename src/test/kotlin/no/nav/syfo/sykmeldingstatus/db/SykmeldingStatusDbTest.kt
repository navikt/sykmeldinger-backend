...
class SykmeldingStatusDbTest : FunSpec({
    ...
    test("test sykmeldingstatus with wrong fnr") {
        TestDB.database.insertStatus("1", getStatus(StatusEventDTO.APEN.name, OffsetDateTime.now().minusDays(1)))
    ...
    test("test get latest avvist sykmeldingstatus") {
        val apenStatus = getStatus(StatusEventDTO.APEN.name, OffsetDateTime.now().minusDays(1))
        TestDB.database.insertStatus("1", getStatus(StatusEventDTO.APEN.name, OffsetDateTime.now().minusDays(1)))
        TestDB.database.insertBehandlingsutfall("1", getBehandlingsutfall(RegelStatusDTO.INVALID))
        TestDB.database.insertSymelding("1", "fnr", getSykmelding())
    ...
    test("update and get latest sykmeldingstatus SENDT") {
        ...
        TestDB.database.insertBehandlingsutfall("1", getBehandlingsutfall(RegelStatusDTO.OK))
        TestDB.database.insertSymelding("1", "fnr", getSykmelding())
        TestDB.database.insertStatus("1", getStatus(StatusEventDTO.SENDT.name, OffsetDateTime.now()))
        val status = database.getLatestStatus("1", "fnr")
    ...
    test("update and get latest sykmeldingstatus BEKREFTET") {
        ...
        TestDB.database.insertBehandlingsutfall("1", getBehandlingsutfall(RegelStatusDTO.OK))
        TestDB.database.insertSymelding("1", "fnr", getSykmelding())
        TestDB.database.insertStatus("1", getStatus(StatusEventDTO.BEKREFTET.name, OffsetDateTime.now().minusHours(2)))
        val status = database.getLatestStatus("1", "fnr")
...
private fun getSykmeldingStatusEvent() = SykmeldingStatusKafkaEventDTO(
    sykmeldingId = "1",
